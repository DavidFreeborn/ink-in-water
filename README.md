...
Initial current strength varies continuously from still water to three times the default. Sixteen seeded, divergence-free Fourier modes define the starting flow. Density contrast can be positive, zero or negative. Add up to five inks, each with an independent density and colour, moving through the same fluid. Multiple drops start well apart, with three to five arranged for clear separation in the initial view. Select an ink to edit it using the colour picker or an exact six-digit hex value. Colour changes absorption only. More inks increase graphics memory and computation; the solver retains the selected resolution and tracer detail. The default remains a single blue ink.

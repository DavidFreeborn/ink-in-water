...
For development, install the test tools with `npm ci` and `npx playwright install chromium`. Run `npm start` to serve the project and, in another terminal, `npm test` to reproduce the numerical and interaction checks in `tests/`. The tests cover conservation, pressure projection, Brownian diffusion, wall containment, torus seams, repeatable seeds, playback and independent ink densities/colours. Measurements are written to `qa/`. The Windows test configuration requests the D3D11 graphics backend; use your platform's hardware backend when adapting it elsewhere.

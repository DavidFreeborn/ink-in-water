...
const hook=String.raw`qaConfigure(mode,inkCount=1){running=false;boundary=mode;containerShape='cuboid';initialMotion='still';currentStrength=0;inks=Array.from({length:inkCount},(_,i)=>({id:i+1,density:1.2,colour:DEFAULT_COLOUR}));activeInkIndex=0;nextInkId=inkCount+1;chooseSeed(12345);if(scalarGroups.length!==Math.ceil(inkCount/4))allocate(quality);else reset();},
qaManufactured(kind){
 ...
 if(!programs.qaScalar)program('qaScalar',\`uniform int pattern;uniform vec4 referenceConcentration;void main(){ivec3 q=cell();if(q.z>=int(n.z)){outColor=vec4(0);return;}q=scalarCell(q);float modulation=pattern==1?1.+.4*sin(6.283185307*(float(q.x)-.5)/(n.x-2.)):1.;outColor=referenceConcentration*modulation;}\`);
 ...
 for(const group of scalarGroups){
  const referenceConcentration=Array.from({length:4},(_,channel)=>group.index*4+channel<inks.length?.25+.125*(group.index*4+channel):0);
  draw('qaScalar',group.dye[0],{},{pattern:kind==='pattern'?1:0,referenceConcentration});
  group.means=referenceConcentration;group.baseMasses=referenceConcentration.map(value=>value*dims.fluidCount*h**3);
 }
 inks.forEach((ink,i)=>{ink.density=kind==='constant'?(i%2?-.4:.8):0;});simTime=0;stepIndex=0;
 if(kind==='gradient')projectVelocity(true);render();return reference;
},
qaBoundaryStats(reference=[0,0,0]){
 const vr=read(velocity[0]),concentrations=scalarGroups.map(group=>read(group.dye[0])),N=dims.active,[nx,ny]=dims.n,columns=dims.columns,width=velocity[0].width;
 ...
 for(let z=0;z<=N[2]+1;z++)for(let y=0;y<=N[1]+1;y++)for(let x=0;x<=N[0]+1;x++){
  ...
  else{const sc=q.map((v,j)=>boundary==='periodic'?wrap(q,j):Math.max(1,Math.min(N[j],v)));for(let species=0;species<inks.length;species++){const cr=concentrations[Math.floor(species/4)],channel=species%4;ghostScalarError=Math.max(ghostScalarError,Math.abs(cr[i+channel]-cr[offset(sc)+channel]));}
...
qaMinimalImage(){
 if(!particleRendering)return{available:false};tracerSets.forEach(set=>{set.count=0;});const set=tracerSets[0];set.count=1;
 gl.bindTexture(gl.TEXTURE_2D,set.particles[0].tex);gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,1,1,gl.RGBA,gl.FLOAT,new Float32Array([.0001,.06,.04,1e-7]));
 gl.bindTexture(gl.TEXTURE_2D,set.particles[1].tex);gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,1,1,gl.RGBA,gl.FLOAT,new Float32Array([.0799,.06,.04,1e-7]));
...
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-angle=d3d11']});try{
 ...
 // Five species exercise all RGBA channels and a second concentration group.
 await page.evaluate(()=>InkSimulation.qaConfigure('periodic',5));

...
const hook=String.raw`qaTwoConfigure(options={}){
 running=false;secondInk=true;containerShape=options.shape||'cuboid';boundary=options.boundary||'container';
 currentStrength=options.strength??0;initialMotion=currentStrength?'gentle':'still';density=options.density??.4;density2=options.density2??-.4;
 inkColour='blue';inkColour2='amber';chooseSeed(125);reset();
},async qaTwoFingerprint(){
 const result=[];for(const field of [dye[0],velocity[0],particles[0],particleIds]){
 ...
 const raw=read(dye[0]),mask=read(levels[0].geometry),N=dims.active,[nx,ny]=dims.n,columns=dims.columns,width=dye[0].width;
 ...
 const scalar=Array.from({length:2},()=>({mass:0,min:Infinity,max:-Infinity,centroid:[0,0,0],solidMaximum:0,finite:true}));
 for(let z=1;z<=N[2];z++)for(let y=1;y<=N[1];y++)for(let x=1;x<=N[0];x++){
  const q=[x,y,z],i=offset(q);for(let species=0;species<2;species++){
   const amount=raw[i+species],s=scalar[species];s.mass+=amount;s.min=Math.min(s.min,amount);s.max=Math.max(s.max,amount);s.finite&&=Number.isFinite(amount);
 ...
 const tracers=Array.from({length:2},()=>({mass:0,count:0,centroid:[0,0,0],outside:0,nonpositive:0,finite:true,weightHash:2166136261,idHash:2166136261}));
 if(particleRendering){const raw=read(particles[0]),bits=new Uint32Array(raw.buffer),ids=read(particleIds);for(let i=0;i<raw.length;i+=4){
  const weight=raw[i+3];if(weight===0)continue;const species=ids[i]>=particleSide**3?1:0,t=tracers[species];t.count++;t.mass+=weight;t.weightHash=Math.imul(t.weightHash^bits[i+3],16777619)>>>0;t.idHash=Math.imul(t.idHash^ids[i],16777619)>>>0;
...
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-angle=d3d11']});try{
 ...
 await page.evaluate(()=>InkSimulation.qaTwoConfigure({density:0,density2:0}));await page.evaluate(()=>InkSimulation.step(100));
 ...
 await page.evaluate(()=>InkSimulation.qaTwoConfigure({density:0,density2:.4}));const secondInitial=await page.evaluate(()=>InkSimulation.qaTwoStatistics());await page.evaluate(()=>InkSimulation.step(100));
 const secondAfter=await page.evaluate(()=>InkSimulation.qaTwoStatistics()),secondFlow=await page.evaluate(()=>InkSimulation.diagnostics());
 ...
 await page.evaluate(()=>InkSimulation.qaTwoConfigure());const oppositeInitial=await page.evaluate(()=>InkSimulation.qaTwoStatistics());await page.evaluate(()=>InkSimulation.step(100));
 const oppositeAfter=await page.evaluate(()=>InkSimulation.qaTwoStatistics());assertConservation(oppositeInitial,oppositeAfter,'Opposite buoyancy');
 ...
 const beforeColours=await page.evaluate(()=>InkSimulation.qaTwoFingerprint());
 await page.locator('#ink-colour').selectOption('red');await page.locator('#ink-colour-2').selectOption('green');
 const afterColours=await page.evaluate(()=>InkSimulation.qaTwoFingerprint());assert.deepEqual(afterColours,beforeColours,'Colour controls leave every physics texture bit-identical');
 await page.evaluate(()=>InkSimulation.step(30));const colouredFuture=await page.evaluate(()=>InkSimulation.qaTwoFingerprint());
 await page.evaluate(()=>{InkSimulation.qaTwoConfigure();InkSimulation.step(130);});const originalFuture=await page.evaluate(()=>InkSimulation.qaTwoFingerprint());
 ...
 for(const [shape,boundary]of [['cuboid','container'],['cylinder','container'],['sphere','container'],['sphere','periodic']]){
  const label=boundary==='periodic'?'torus':shape;await page.evaluate(options=>InkSimulation.qaTwoConfigure(options),{shape,boundary,strength:3,density:1.2,density2:-.4});
  const initial=await page.evaluate(()=>InkSimulation.qaTwoStatistics());for(let batch=0;batch<5;batch++)await page.evaluate(()=>InkSimulation.step(100));
  const after=await page.evaluate(()=>InkSimulation.qaTwoStatistics()),flow=await page.evaluate(()=>InkSimulation.diagnostics());assertConservation(initial,after,label+' maximum-current run');
 ...
 assert.deepEqual(errors,[]);report.errors=errors;fs.writeFileSync('qa/two-inks-validation.json',JSON.stringify(report,null,2));
